...
INPUT_TTL   = int(os.getenv("INPUT_FILE_TTL_SECONDS", "1800"))     # Standard: 30 Minuten
...
async def convert_to_mp3(ctx, job_id: str, in_path: str):
    ...
    try:
        ...
        # Input-Datei NICHT sofort löschen – TTL setzen, Cleanup räumt auf
        await redis.set(f"job:{job_id}:input_path", in_path)
        await redis.set(f"job:{job_id}:input_expires_at", str(time.time() + INPUT_TTL))


async def cleanup_old_files(ctx):
    """
    Scheduled Task – läuft alle 15 Minuten.
    Löscht MP3-Dateien, Input-Dateien und Redis-Keys deren TTL abgelaufen ist.
    ...
    # --- Input-Dateien (Video/Audio) aufräumen ---
    async for key in redis.scan_iter("job:*:input_expires_at"):
        expires_at = await redis.get(key)
        if not expires_at:
            continue
        ...
        if float(expires_at) < now:
            job_id = key.decode().split(":")[1]
            input_path = await redis.get(f"job:{job_id}:input_path")

            if input_path:
                try:
                    os.remove(input_path.decode())
                    deleted += 1
                except FileNotFoundError:
                    pass

            # Input-spezifische Redis-Keys löschen
            await redis.delete(
                f"job:{job_id}:input_path",
                f"job:{job_id}:input_expires_at"
            )
